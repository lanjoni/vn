...
func (rg *ReportGenerator) GenerateReport() PerformanceReport {
	...
	if len(successfulMetrics) > 0 {
		...
		report.MedianExecutionTime = calculateMedianDuration(successfulMetrics, func(m TestMetrics) time.Duration { return m.ExecutionTime })
		report.P95ExecutionTime = calculateP95Duration(successfulMetrics, func(m TestMetrics) time.Duration { return m.ExecutionTime })
	}
...
func (rg *ReportGenerator) GenerateRegressionReport(detector *RegressionDetector, thresholdPercent float64, writer io.Writer) error {
